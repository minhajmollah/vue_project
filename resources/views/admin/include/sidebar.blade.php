        ...
        <ul class="nav metismenu" id="side-menu">
            <li class="nav-header">
                <div class="dropdown profile-element">
                    @if(Auth::guard('admin')->user()->avatar)
                        <img alt="Avatar" style="max-width: 80px;" class="img-fluid rounded-circle"
                             src="{{ asset('images/admin/'.Auth::guard('admin')->user()->avatar) }}"/>
                    @else
                        <img alt="Avatar" style="max-width: 80px;" class="img-fluid rounded-circle"
                    ...
                    <a data-toggle="dropdown" class="dropdown-toggle" href="#">
                        ...
                        <span class="text-muted text-xs block">{{ Auth::guard('admin')->user()->role->role_name }}<b
                                class="caret"></b></span>
                    </a>
            ...
            @foreach($side_menu as $value)
                ...
                @if(count($value['sub_menu'])>0)
                    <li class="parent">
                        <a href="#"><i class="fa {{ $value['icon'] }}"></i> <span
                                class="nav-label">{{ $value['name'] }}</span> <span class="fa arrow"></span></a>
